...
public class CCLCompat {
    ...
    private static BlockRendererRegistry.Renderer createBridge(ICCBlockRenderer r) {
        return ccRendererToSodium.computeIfAbsent(r, ccRenderer -> (state, pos, world, consumer, random, modelData, layer) -> {
            ccRenderer.renderBlock(state, pos, world, STACK_THREAD_LOCAL.get(), consumer, random, modelData, layer);
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        if(ModList.get().isLoaded("codechickenlib")) {
            init();
            BlockRendererRegistry.instance().registerRenderPopulator((resultList, state, pos, world, layer) -> {
                if(!customGlobalRenderers.isEmpty()) {
                    for(ICCBlockRenderer r : customGlobalRenderers) {
                        if(r.canHandleBlock(world, pos, state, layer)) {
                ...
                if(!customBlockRenderers.isEmpty()) {
                    ...
                    for(Map.Entry<RegistryEntry<Block>, ICCBlockRenderer> entry : customBlockRenderers.entrySet()) {
                        if(entry.getKey().get() == block && entry.getValue().canHandleBlock(world, pos, state, layer)) {
                ...
                if(!customFluidRenderers.isEmpty()) {
                    ...
                    for(Map.Entry<RegistryEntry<Fluid>, ICCBlockRenderer> entry : customFluidRenderers.entrySet()) {
                        if(entry.getKey().get().matchesType(fluid) && entry.getValue().canHandleBlock(world, pos, state, layer)) {
                            resultList.add(createBridge(entry.getValue()));
                        }
                    }
                }
