...
public class IncompatibleModManager {
    ...
    public static void checkMods(FMLClientSetupEvent event) {
        ...
        if(modDeclarationList.length > 0) {
            event.enqueueWork(() -> {
                ModLoader.addLoadingIssue(ModLoadingIssue.warning("embeddium.conflicting_mod", String.join(", ", modDeclarationList)));
                ModLoader.addLoadingIssue(ModLoadingIssue.warning("embeddium.conflicting_mod_list", String.join(", ", modDeclarationList)));
            });
        }
    }
}
