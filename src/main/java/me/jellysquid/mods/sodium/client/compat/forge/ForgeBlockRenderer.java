...
public class ForgeBlockRenderer {
    ...
    public boolean renderBlock(LightMode mode, BlockState state, BlockPos pos, BlockAndTintGetter world, BakedModel model, PoseStack stack,
        ...
        if (mode == LightMode.FLAT) {
            forgeRenderer.renderModelFlat(world, model, state, pos, stack, buffer, checkSides, random, seed, OverlayTexture.NO_OVERLAY, data);
        } else {
            forgeRenderer.renderModelSmooth(world, model, state, pos, stack, buffer, checkSides, random, seed, OverlayTexture.NO_OVERLAY, data);
        }
