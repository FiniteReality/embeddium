...
public class ChunkBufferSorter {
    public static void sort(ChunkMeshData chunkData, float x, float y, float z) {
        ...
        ByteBuffer buffer = chunkData.getVertexData().vertexBuffer().getDirectBuffer();
        // Quad stride by Float size
        int quadStride = chunkData.getVertexData().vertexFormat().getStride();

        int quadStart = ((Buffer)buffer).position();
        int quadCount = buffer.capacity()/quadStride/4;
